...
public:
    queue(size_t capacity) : N(capacity), frontInd(0), rearInd(0) {
        arr = new T[N];
    }
    ...
    void push(const T& val) {
        if (rearInd == N) throw overflow_error("Queue is full!");

        arr[rearInd++] = val;
    }

    void pop() {
        if (frontInd == rearInd)  throw underflow_error("Queue is empty!");
