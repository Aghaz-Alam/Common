...
class KeyValueDB{
    //implement class
    public:
      //my own implementation let me know if it's correct
...
int main(int argc, char** argv) {
    // Integer Key-Value Database
    KeyValueDB<int> intDB;

    intDB.set("one", 1);
    intDB.set("two", 2);
    intDB.set("three", 3);

    cout << "intDB size: " << intDB.size() << endl;

    if (auto val = intDB.get("two")) {
        cout << "Value of 'two': " << *val << endl;
    }

    cout << "Exists 'three'? " << intDB.exists("three") << endl;
    cout << "Exists 'four'? " << intDB.exists("four") << endl;

    intDB.remove("one");
    cout << "intDB size after remove: " << intDB.size() << endl;

    // String Key-Value Database
    KeyValueDB<string> strDB;

    strDB.set("name", "Aghaz");
    strDB.set("language", "C++");

    cout << "strDB size: " << strDB.size() << endl;

    if (auto val = strDB.get("name")) {
        cout << "Value of 'name': " << *val << endl;
    }

    strDB.clear();
    cout << "strDB size after clear: " << strDB.size() << endl;
...
Exists 'three'? 1
Exists 'four'? 0
intDB size after remove: 2
strDB size: 2
Value of 'name': Aghaz
strDB size after clear: 0
*/
